...
module Smpp
  ...
  class Base < EventMachine::Connection
    ...
    def read_pdu(data)
      pdu = nil
      # we may either receive a request or a response.
      begin
        header = data[0..15]
        if !header
          # EOF reached: stop the event loop so we can reconnect
          EventMachine::stop_event_loop and return
        end
        len, cmd, status, seq = header.unpack('N4')
        body = data[16..-1]
        pdu = Pdu::Base.create(len, cmd, status, seq, body)
        if !pdu
